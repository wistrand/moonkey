...
- **Configurable via the native watch-face editor** (fenix 8+ / CIQ 5.1.0; `marq2` compiles but has no editor): `resources/watchface-config.xml` exposes **accent colour**, **data colour** (`_dataColor`, the N/E/S/W readouts + weather glyphs/barb/precip), a **timezone** picker (the `<styles>` selector repurposed — `TZ_OFFSET`/`TZ_LABEL`/`TZ_DST`, array index == style id), and **5 complication slots** (`SLOT_SE/NE/N/S/NW` == `<complication id>`s). `AnalogWatchFaceDelegate` previews edits live (`onWatchFaceConfigEdited`) and maps **taps on a field to its complication** (`onTap`→`getTappedComplication`→`setSelectedComplication`) so the user picks by tapping the face, not by numeric id.
- **Also Connect IQ app settings** (`resources/settings/`, editable in Garmin Connect for beta/store installs — the editor-less path for marq2): same accent/data colour (curated lists), timezone, and complications (per-slot type dropdowns, `compTypeFromCode`). `applyProperties()` reads `Application.Properties` and drives the **same setters** the native editor does; `MoonkeyApp.onSettingsChanged()` re-applies on sync. **Single binary, sentinel precedence:** native config first, then app settings override only where a property differs from its `-1`/`0` "unset" default — so the native editor wins unless the user changed a Connect setting.
