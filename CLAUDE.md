...
**Full design: [agent_docs/architecture.md](agent_docs/architecture.md).** Configurability research (accent colour / timezone, sideload constraints): [agent_docs/finding-config.md](agent_docs/finding-config.md).
...
- **Moon** (see architecture.md): soft-terminator shaded photo baked into a cached `BufferedBitmap` hourly, then blitted **rotated by sky inclination** (bright-limb PA − parallactic angle, computed in-code). Tilt recomputed every frame (cheap) so it tracks position/time; `MOON_TILT_OFFSET` calibrates the baked orientation to the sky.
