...
## Assets to upload
Generated by `make store-assets` (icon + on-device + hero) and `make gallery` (screenshots), no hand editing. The **store icon and hero are real watch-face screenshots** (the CIQ-store convention), cut out of the gallery renders by `scripts/cut-face.sh` (crop the device screen rect from `simulator.json` + circular mask). The on-device icon stays a designed crescent glyph (it is the in-watch app-list icon, not a shot):

- **Store icon** (500×500): the round face on transparent, cut from `cfg-default` (marq2aviator). [`docs/store-icon.png`](../docs/store-icon.png) · `make store-icon`. Note: it is a transparent black round face, so on a dark store theme it can blend; the store renders cover images on a light card, where it reads fine.
...
- **Hero** (1440×720): three faces on a dark navy-to-black radial gradient (default centred, fox and loaded flanking). [`docs/store-hero.png`](../docs/store-hero.png) · `make hero`
