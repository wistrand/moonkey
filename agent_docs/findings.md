...
## Moon position dot jumped ~50 min backwards "after midnight" (2026-06-12)

**Symptom:** fast-forwarding time in the sim, the moon position dot (its E-W separation from the sun, on the day/night ring) hopped backwards ~12 deg once a day, around "after midnight".

**Diagnosis:** the dot's angle used the moon's transit time (`now - transit_moon + transit_sun`). `Astro.moonRiseSet` finds the transit by Newton **seeded at `now`** with the hour angle wrapped to +/-12h, so `_mrsTransit` is the transit *nearest now*. When `now` crosses the moon's anti-transit (~12h25m after transit, which is near midnight when the moon transits around midday), the nearest transit flips to the next one and `_mrsTransit` jumps ~50 min (one lunar day, mod 24). The dot's gap is thus quantized to a daily transit value and steps ~50 min at the flip; on the ring that's a ~12 deg backward hop. (Real time: once a day, easy to miss; fast-forward made it obvious.)

**Fix:** drive the dot from the **RA elongation** instead. `_moonElongRad = normalize_to_+/-pi(moonRaDec[0] - sunRaDec[0])`, cached hourly in `ensureMrs`, geocentric (needs no location). The dot angle is `now*pi/12 - _moonElongRad`, so the gap to the now-pointer is the true moon-sun separation, smooth and continuous with no transit-flip. Same meaning as before (transit-time difference ~= RA difference), just not quantized to daily transit values. `_mrsTransit` was dropped from the view; the moon arc uses rise/set, not the transit.
