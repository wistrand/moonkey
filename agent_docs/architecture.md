...
- **Data fields** — four cardinal readouts (heart rate, steps, weather, time+date) and four diagonal radial-text fields (intensity, floors, UTC, sunrise/sunset), the latter drawn on a curve via a vector font (`drawRadialText`) and cached in a pool buffer (see Performance). The **weather** field stacks an icon over the temperature with a thin **precip-chance bar** between them, and a **wind barb** to its left:
